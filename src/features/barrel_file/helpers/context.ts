...
class GeneratorContext {
  ...
  constructor() {
    this.channel = window.createOutputChannel('BarrelFile');

    this.channel.appendLine(
      `[${formatDate()}] BarrelFile extension enabled`
    );
  ...
  initGeneration({ path, type }: InitParms): void {
    ...
    this.channel.appendLine(
      `[${formatDate()}] Type: ${type ? 'recursive' : 'regular'} - Path: ${path.fsPath
      }`
    );
  }
  ...
  deactivate() {
    this.channel.appendLine(
      `[${formatDate()}] BarrelFile extension disabled`
    );
  ...
  writeFolderInfo({ path, fileCount }: WriteInfoParams) {
    this.channel.appendLine(
      `[${formatDate()}] Exporting ${path} - found ${fileCount} files`
    );
  }
