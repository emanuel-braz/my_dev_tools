...
import BarrelFileUtil from './barrel_file_util';
...
export const init = async () => {
  if (!Context.activeType) {
    ...
    window.showErrorMessage('MDT: Error on initialising the extension');
  }

  try {
    const buttonLabel = 'Open generated file';
    ...
    if (maybeGenerated) {
      ...
      if (result === buttonLabel) {
        workspace.openTextDocument(maybeGenerated).then(window.showTextDocument);
      }
    } else {
      await window.showErrorMessage(
        'MDT: No barrel file has been generated!'
      );
    }
    ...
    window.showErrorMessage('MDT: Error on generating the file', error);
  }
};
...
const writeBarrelFile = (
  ...
  const languageDetected = BarrelFileUtil.detectLanguage(targetPath);

  let exports = '';

  let prependPackageValue = '';

  if (languageDetected === '.dart') {
    ...
    prependPackageValue = shouldPrependPackage
  ...
  for (const t of files) {
    switch (languageDetected) {
      case '.dart':
        ...
        break;
      case '.ts':
        exports = `${exports}export * from './${t.replace(/\.[^/.]+$/, '')}';\n`;
        break;
      case '.kt':
        exports = `${exports}export * from './${t.replace(/\.[^/.]+$/, '')}';\n`;
        break;
      case '.rs':
        exports = `${exports}pub mod ${t.replace(/\.[^/.]+$/, '')};\n`;
        break;
      default:
        exports = `${exports}export '${prependPackageValue}${t}';\n`;
        break;
  ...
  const barrelFile = `${targetPath}/${dirName}${languageDetected}`;
